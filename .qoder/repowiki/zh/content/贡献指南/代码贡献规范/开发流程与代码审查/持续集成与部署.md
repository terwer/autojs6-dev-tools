...
- [scripts/release/New-CodeSigningCertificate.ps1](file://scripts/release/New-CodeSigningCertificate.ps1)
- [scripts/release/Test-PortablePackageSmoke.ps1](file://scripts/release/Test-PortablePackageSmoke.ps1)
</cite>

## 更新摘要
**所做更改**
- 更新MSIX包装在CI中的临时禁用状态，明确当前支持的发布输出仅限ZIP和EXE安装器
- 补充CI安全策略章节，详细说明MSIX在CI中的特殊处理机制
- 完善故障排查指南，增加MSIX相关问题的诊断方法
...
- 打包与安装器
  ...
  - MSIX 包：Windows 应用商店兼容包（**当前在CI中临时禁用**）
  - ZIP 便携包：直接解压运行
- 测试体系
...
GH->>Test : 运行单元测试
Test-->>GH : 测试报告
GH->>Pack : 生成 ZIP/EXEMSIX在CI中临时禁用
Pack-->>GH : 产物清单与校验
GH->>Rel : 上传产物可选：正式发布
Rel-->>Dev : 下载与验证
```
...
- 产出 ZIP、EXE、MSIX 三类安装包（**MSIX在CI中临时禁用**）
...
NoUpload --> BuildAll["构建 ZIP/EXEMSIX临时禁用"]
Upload --> BuildAll
...
PR->>GH : 触发正式发布
GH->>Rel : 创建/更新 Release 并上传产物MSIX临时禁用
Rel-->>Dev : 发布完成
```
...
- 安装包格式
  ...
  - MSIX：现代 Windows 应用包，便于分发与更新（**当前在CI中临时禁用**）
- 打包脚本与配置
...
- **新增** CI 安全依赖
  ...
  - **MSIX 包装脚本在 CI 中被临时禁用，通过 BUILD-INFO.txt 文件标记状态**
...
RELEASE["release-please.yml"]
SEC["安全策略审查"]
MSIX_SCRIPT["Build-MsixPackage.ps1"]
MANUAL --> APP
RELEASE --> APP
MANUAL --> SEC
RELEASE --> SEC
...
- **MSIX 打包在 CI 中临时禁用**
  ...
  - **通过 BUILD-INFO.txt 文件明确标记 MSIX 状态为 "temporarily_disabled"**
...
- **新增** MSIX 相关问题
  ...
  - 检查 BUILD-INFO.txt 文件中的 msix_status 字段
  - 等待证书信任验证完成后重新启用
- **新增** BUILD-INFO.txt 文件检查
  - 查看 msix_status 字段值是否为 "temporarily_disabled"
  - 确认工作流中未尝试构建 MSIX 包
...
## 结论
通过 manual-release-test 与 release-please 的双轨流程，AutoJS6 开发工具实现了"先验证、后发布"的稳健 CI/CD 实践。结合多平台与多格式打包策略，配合严格的证书与签名管理，确保了从开发到发布的全流程可控、可追溯与可复现。**新增的 CI 无交互安全指南进一步强化了自动化流程的稳定性，通过脚本交互性分析和故障排查机制，确保发布流程在各种环境中都能可靠运行。当前 MSIX 包装在 CI 中的临时禁用状态已明确记录，确保团队成员了解当前的发布能力边界。**
