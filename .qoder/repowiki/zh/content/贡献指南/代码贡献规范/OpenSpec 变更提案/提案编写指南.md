...
1. **现有项目理解**：深入分析 yxs-day-task 项目的业务逻辑和现有脚本实现
2. **AutoJS6 生态研究**：查阅官方文档和源码，理解 API 约束和技术限制
3. **MVP 验证**：通过 MVP 项目验证核心技术栈的可行性

```mermaid
flowchart TD
Start([开始提案准备]) --> ReadProject["阅读 yxs-day-task 项目<br/>理解业务逻辑"]
ReadProject --> StudyDocs["研究 AutoJS6 文档<br/>理解 API 约束"]
StudyDocs --> AnalyzeScripts["分析现有脚本<br/>capture-current.cmd / generate-region-ref / matchReferenceTemplate"]
