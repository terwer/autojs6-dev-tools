# 快速开始

<cite>
**本文引用的文件**
- [README.md](file://README.md)
- [README_zh_CN.md](file://README_zh_CN.md)
- [DEVELOPMENT.md](file://DEVELOPMENT.md)
- [AGENTS.md](file://AGENTS.md)
- [autojs6-dev-tools.slnx](file://autojs6-dev-tools.slnx)
- [App/App.csproj](file://App/App.csproj)
- [Core/Core.csproj](file://Core/Core.csproj)
- [Infrastructure/Infrastructure.csproj](file://Infrastructure/Infrastructure.csproj)
- [App/Properties/launchSettings.json](file://App/Properties/launchSettings.json)
- [App/App.xaml.cs](file://App/App.xaml.cs)
- [App/App.xaml](file://App/App.xaml)
- [App/MainWindow.xaml.cs](file://App/MainWindow.xaml.cs)
- [App/MainWindow.xaml](file://App/MainWindow.xaml)
- [openspec/project.md](file://openspec/project.md)
- [App.Tests/App.Tests.csproj](file://App.Tests/App.Tests.csproj)
- [Core.Tests/Core.Tests.csproj](file://Core.Tests/Core.Tests.csproj)
- [Core/Models/AdbDevice.cs](file://Core/Models/AdbDevice.cs)
- [Infrastructure/Adb/AdbServiceImpl.cs](file://Infrastructure/Adb/AdbServiceImpl.cs)
</cite>

## 更新摘要
**变更内容**
- 新增Bilibili快速开始视频教程，提供完整的从安装到使用的视频指导
- 增强ADB安装和配置文档，明确ADB在连接Android设备时的必要性
- 改进系统要求说明，突出.NET 8 SDK和ADB作为必备前置条件
- 优化动机解释，更清晰地阐述工具解决的核心痛点
- 更新环境变量配置说明，聚焦于AUTOJS6相关的两个核心环境变量

## 目录
1. [简介](#简介)
2. [B站快速开始视频](#b站快速开始视频)
3. [项目结构](#项目结构)
4. [核心组件](#核心组件)
5. [架构总览](#架构总览)
6. [详细组件分析](#详细组件分析)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本指南面向首次接触 AutoJS6 可视化开发工具的开发者，帮助你在 Windows 10/11 上快速完成环境准备、项目克隆、依赖还原、本地路径配置与运行。该工具专为解决AutoJS6脚本开发中的痛点而设计，能够显著提升开发效率和质量。

**为什么你需要这个工具？**
- **图像识别调试困难**：模板匹配经常失败，需要多次试错才能找到合适阈值
- **坐标定位不准**：手动猜测坐标，经常出现偏差导致点击失败
- **跨设备兼容性差**：不同分辨率设备上脚本表现不一致
- **开发流程繁琐**：截图→裁剪→写代码→设备测试→反复调试的循环

**工具如何解决这些问题？**
- 实时模板匹配预览，支持阈值实时调节
- 可视化坐标拾取，精确到像素级别的定位
- 自动代码生成，一键生成可运行的AutoJS6脚本
- 多分辨率测试支持，一次调试多设备适配

## B站快速开始视频

**强烈推荐先观看B站快速开始视频**，视频将完整演示从.NET 8安装到首次使用的所有关键步骤，包括项目克隆、依赖安装、ADB配置、截图加载、模板裁剪、匹配预览和代码生成等完整流程。

### 视频内容概览
- **.NET 8 SDK安装与验证**
- **项目克隆与依赖还原**
- **ADB工具安装与配置**
- **首次启动应用**
- **截图加载与模板裁剪**
- **实时匹配预览与阈值调节**
- **AutoJS6代码自动生成**

### 视频资源
- **B站快速开始视频**：[AutoJS6 可视化开发工具包快速上手](https://space.bilibili.com/19450592)

**视频优势**：
- 完整的端到端演示流程
- 减少试错成本，快速上手
- 适合初学者的友好指导
- 实际操作场景演示

## 项目结构
该项目采用分层架构，核心模块包括：
- App：WinUI 3 应用层，负责 UI 与 MVVM
- Core：纯业务逻辑层，无 UI 依赖，独立可测试
- Infrastructure：封装外部依赖（ADB、OpenCV、ImageSharp）

```mermaid
graph TB
subgraph "解决方案"
SLNX["autojs6-dev-tools.slnx"]
end
subgraph "应用层"
APP["App/App.csproj<br/>.NET 8 + WinUI 3"]
end
subgraph "基础设施层"
INFRA["Infrastructure/Infrastructure.csproj<br/>.NET 8 + OpenCV + ADB"]
end
subgraph "核心业务层"
CORE["Core/Core.csproj<br/>.NET 8 + 纯业务逻辑"]
end
TEST_APP["App.Tests/App.Tests.csproj"]
TEST_CORE["Core.Tests/Core.Tests.csproj"]
SLNX --> APP
SLNX --> INFRA
SLNX --> CORE
SLNX --> TEST_APP
SLNX --> TEST_CORE
APP --> INFRA
INFRA --> CORE
```

**图表来源**
- [autojs6-dev-tools.slnx:1-34](file://autojs6-dev-tools.slnx#L1-L34)
- [App/App.csproj:1-88](file://App/App.csproj#L1-L88)
- [Infrastructure/Infrastructure.csproj:1-19](file://Infrastructure/Infrastructure.csproj#L1-L19)
- [Core/Core.csproj:1-10](file://Core/Core.csproj#L1-L10)
- [App.Tests/App.Tests.csproj:1-17](file://App.Tests/App.Tests.csproj#L1-L17)
- [Core.Tests/Core.Tests.csproj:1-21](file://Core.Tests/Core.Tests.csproj#L1-L21)

**章节来源**
- [README.md:262-292](file://README.md#L262-L292)
- [autojs6-dev-tools.slnx:1-34](file://autojs6-dev-tools.slnx#L1-L34)

## 核心组件
- 应用层（App）：WinUI 3 + Windows App SDK，启用 MSIX 打包与 Win2D 渲染，支持多平台运行时标识（win-x86/win-x64/win-arm64）
- 基础设施层（Infrastructure）：封装 ADB 通信、OpenCV 图像处理、ImageSharp 图像操作
- 核心业务层（Core）：纯业务逻辑，独立于 UI 与外部依赖，便于单元测试

**章节来源**
- [App/App.csproj:1-88](file://App/App.csproj#L1-L88)
- [Infrastructure/Infrastructure.csproj:1-19](file://Infrastructure/Infrastructure.csproj#L1-L19)
- [Core/Core.csproj:1-10](file://Core/Core.csproj#L1-L10)

## 架构总览
系统采用 Clean Architecture 分层，单向依赖：App → Infrastructure → Core ← Infrastructure。

```mermaid
graph LR
APP["App<br/>UI + MVVM"] --> INFRA["Infrastructure<br/>外部依赖适配"]
INFRA --> CORE["Core<br/>纯业务逻辑"]
TEST_APP["App.Tests"] -.-> APP
TEST_CORE["Core.Tests"] -.-> CORE
```

**图表来源**
- [App/App.csproj:67-68](file://App/App.csproj#L67-L68)
- [Infrastructure/Infrastructure.csproj:9-11](file://Infrastructure/Infrastructure.csproj#L9-L11)
- [Core/Core.csproj:1-10](file://Core/Core.csproj#L1-L10)

**章节来源**
- [README.md:296-320](file://README.md#L296-L320)

## 详细组件分析

### 系统要求与前置条件

**⚠️ 重要前置条件：.NET 8 SDK 和 ADB 必须先安装**
> 在尝试运行之前，请先安装 .NET 8 SDK 和 ADB  
> 下载地址：https://dotnet.microsoft.com/zh-cn/download/dotnet/8.0  
> ADB下载：Android Studio官方页面或Android SDK Platform-Tools  
> 缺少 .NET 8 或 ADB 可能导致应用无法正常启动或在某些机器上运行异常

- **操作系统**：Windows 10/11（建议 Build 22621.0+）
- **运行时**：.NET 8 SDK（**必备**）
- **开发工具**：Visual Studio 2022/2026（含 WinUI 3 工作负载）
- **设备工具**：Android Debug Bridge（ADB）需在 PATH 中（**必备**）
- **可选**：MSBuild + SignTool、Inno Setup 6（用于本地打包验证）

**为什么需要.NET 8和ADB？**
- 项目使用 .NET 8 作为目标框架（App.csproj:4）
- WinUI 3 需要 .NET 8 运行时支持
- 确保与 Windows App SDK 的兼容性
- 提供更好的性能和稳定性
- ADB用于设备发现、截图捕获和UI层次结构检查

**章节来源**
- [README.md:11-17](file://README.md#L11-L17)
- [README.md:129-135](file://README.md#L129-L135)
- [App/App.csproj:4](file://App/App.csproj#L4)

### ADB安装和配置详细指南

**ADB工具的重要性**
ADB（Android Debug Bridge）是连接Android设备与电脑的关键工具。没有ADB，以下功能将无法使用：
- 设备发现和连接
- 实时截图捕获
- UI层次结构分析
- 自动化测试支持

**ADB安装选项**

**选项一：Android Studio（推荐）**
1. 从官方页面下载并安装Android Studio
2. 安装完成后，ADB会自动包含在Android SDK Platform-Tools中
3. 确保ADB路径已添加到系统PATH环境变量

**选项二：独立安装Android SDK Platform-Tools**
1. 直接下载Android SDK Platform-Tools
2. 解压到指定目录（如 C:\Android\platform-tools）
3. 将该目录添加到系统PATH环境变量

**选项三：通过JetBrains Toolbox安装**
1. 使用JetBrains Toolbox安装Android Studio
2. 在Toolbox中使用内置的SDK Manager安装SDK/Platform-Tools
3. 确保ADB可用

**ADB配置验证**
安装完成后，在命令提示符中输入以下命令验证配置：

```cmd
adb version
adb devices
```

如果命令正常响应，说明ADB已正确安装和配置。

**章节来源**
- [README.md:15-17](file://README.md#L15-L17)
- [README.md:138](file://README.md#L138)

### 安装与配置步骤

#### 步骤一：克隆仓库
- 在终端中执行克隆命令并进入项目目录

**章节来源**
- [README.md:162-167](file://README.md#L162-L167)

#### 步骤二：安装依赖
- 还原 NuGet 包（推荐先还原解决方案）

**章节来源**
- [README.md:169-174](file://README.md#L169-L174)

#### 步骤三：配置本地路径
- 编辑 AGENTS.md 文件，设置以下环境变量（按本机路径替换）
  - AUTOJS6_DOCS_ROOT：AutoJS6 文档根目录
  - AUTOJS6_SOURCE_ROOT：AutoJS6 源码根目录

**更新** 删除了 YXS_DAY_TASK_ROOT 环境变量配置说明，仅保留当前项目所需的 AUTOJS6 环境变量

**章节来源**
- [README.md:176-183](file://README.md#L176-L183)
- [AGENTS.md:99-107](file://AGENTS.md#L99-L107)
- [openspec/project.md:32-39](file://openspec/project.md#L32-L39)

#### 步骤四：构建与运行
- 方式一：命令行
  - 还原解决方案包
  - 构建解决方案
  - 运行应用项目
- 方式二：Visual Studio
  - 打开解决方案文件，直接启动（F5）

**章节来源**
- [README.md:185-198](file://README.md#L185-L198)

### 运行方式对比

```mermaid
flowchart TD
Start(["开始"]) --> Choose["选择运行方式"]
Choose --> CLI["命令行构建运行"]
Choose --> VS["Visual Studio 直接启动"]
CLI --> CLI_Restore["dotnet restore autojs6-dev-tools.slnx"]
CLI_Restore --> CLI_Build["dotnet build autojs6-dev-tools.slnx"]
CLI_Build --> CLI_Run["dotnet run --project App/App.csproj"]
VS --> VS_Open["打开 autojs6-dev-tools.slnx"]
VS_Open --> VS_F5["F5 启动"]
CLI_Run --> End(["完成"])
VS_F5 --> End
```

**图表来源**
- [README.md:185-198](file://README.md#L185-L198)
- [autojs6-dev-tools.slnx:1-34](file://autojs6-dev-tools.slnx#L1-L34)

**章节来源**
- [README.md:185-198](file://README.md#L185-L198)

### 应用启动流程（代码级）

```mermaid
sequenceDiagram
participant User as "用户"
participant AppXaml as "App.xaml.cs"
participant MainWindow as "MainWindow.xaml.cs"
User->>AppXaml : 启动应用
AppXaml->>AppXaml : 初始化应用
AppXaml->>MainWindow : 创建主窗口
MainWindow->>MainWindow : 最大化窗口
MainWindow-->>User : 显示工作台界面
```

**图表来源**
- [App/App.xaml.cs:49-54](file://App/App.xaml.cs#L49-L54)
- [App/MainWindow.xaml.cs:28-50](file://App/MainWindow.xaml.cs#L28-L50)

**章节来源**
- [App/App.xaml.cs:1-57](file://App/App.xaml.cs#L1-L57)
- [App/MainWindow.xaml.cs:1-53](file://App/MainWindow.xaml.cs#L1-L53)

## 依赖分析
- App 依赖 Infrastructure
- Infrastructure 依赖 Core
- 测试项目分别依赖 App 与 Core

```mermaid
graph LR
TEST_APP["App.Tests"] --> APP["App"]
TEST_CORE["Core.Tests"] --> CORE["Core"]
APP --> INFRA["Infrastructure"]
INFRA --> CORE
```

**图表来源**
- [App.Tests/App.Tests.csproj:1-17](file://App.Tests/App.Tests.csproj#L1-L17)
- [Core.Tests/Core.Tests.csproj:1-21](file://Core.Tests/Core.Tests.csproj#L1-L21)
- [App/App.csproj:71](file://App/App.csproj#L71)
- [Infrastructure/Infrastructure.csproj:9-11](file://Infrastructure/Infrastructure.csproj#L9-L11)

**章节来源**
- [App.Tests/App.Tests.csproj:1-17](file://App.Tests/App.Tests.csproj#L1-L17)
- [Core.Tests/Core.Tests.csproj:1-21](file://Core.Tests/Core.Tests.csproj#L1-L21)
- [App/App.csproj:71](file://App/App.csproj#L71)
- [Infrastructure/Infrastructure.csproj:9-11](file://Infrastructure/Infrastructure.csproj#L9-L11)

## 性能考虑
- 异步优先：所有 I/O 操作（ADB、OpenCV、XML 解析、纹理上传）使用异步模型，避免 UI 阻塞
- 渲染优化：Win2D 双层渲染（图像层 + 覆盖层），阈值滑动仅重算匹配层，不重建图像纹理
- 平台配置：针对 win-x86/win-x64/win-arm64 设置明确的运行时标识，避免 AnyCPU 回退

**章节来源**
- [AGENTS.md:209-233](file://AGENTS.md#L209-L233)
- [App/App.csproj:13-18](file://App/App.csproj#L13-L18)

## 故障排除指南
- **.NET 8 SDK 未安装或版本不匹配**
  - 安装 .NET 8 SDK 并确认版本
  - 重启开发环境使新安装生效
- **ADB未安装或配置错误**
  - 确认 ADB 工具已安装并加入系统 PATH
  - 在命令提示符中运行 `adb devices` 验证连接
  - 如无设备显示，检查USB调试设置和驱动程序
- **Visual Studio 缺失 WinUI 3 工作负载**
  - 通过 Visual Studio Installer 安装 WinUI 3 工作负载
- **还原/构建失败**
  - 先执行 dotnet restore，再执行 dotnet build
- **MSIX 打包签名失败**
  - 确认证书主题与发布清单中的 Publisher 一致，导入证书到受信任发布者与受信任根证书颁发机构存储
- **EXE 安装器构建失败**
  - 确认 Inno Setup 6 的 ISCC.exe 可用，输出目录可写

**章节来源**
- [README.md:129-135](file://README.md#L129-L135)
- [DEVELOPMENT.md:118-127](file://DEVELOPMENT.md#L118-L127)
- [DEVELOPMENT.md:390-416](file://DEVELOPMENT.md#L390-L416)

## 结论
按照本指南完成系统要求（特别是.NET 8 SDK和ADB的安装）、克隆仓库、还原依赖、配置本地路径与运行方式后，你将能在 Windows 上顺利启动 AutoJS6 可视化开发工具。**强烈建议先观看B站快速开始视频**，它将提供完整的端到端演示，帮助你从"刚下载"快速进入"真正能用"的状态。该工具能够显著提升你的开发效率，解决AutoJS6脚本开发中的常见痛点，让你专注于业务逻辑而非繁琐的调试过程。

## 附录

### 快速命令清单
- 克隆与进入目录
- 还原解决方案包
- 构建解决方案
- 运行应用项目
- 打开解决方案并在 Visual Studio 中启动

**章节来源**
- [README.md:162-198](file://README.md#L162-L198)

### 环境变量配置详解

#### AUTOJS6 环境变量说明
项目当前仅需要以下两个环境变量来支持本地 AutoJS6 API 和源码查找：

- **AUTOJS6_DOCS_ROOT**：指向 AutoJS6 文档根目录的本地路径
  - 用途：API 发现、引用和辅助阅读
  - 路径结构：`$AUTOJS6_DOCS_ROOT/json/`（API 发现）、`$AUTOJS6_DOCS_ROOT/api/`（API 引用）、`$AUTOJS6_DOCS_ROOT/docs/`（辅助阅读）

- **AUTOJS6_SOURCE_ROOT**：指向 AutoJS6 源码根目录的本地路径
  - 用途：最终 API 定义和实现参考
  - 关键源码位置：
    - `app/src/main/java/org/autojs/autojs/runtime/api/augment/`：API 实现
    - `app/src/main/java/org/autojs/autojs/core/accessibility/`：无障碍桥接层
    - `app/src/main/java/org/autojs/autojs/core/activity/`：前台应用检测

**更新** 删除了 YXS_DAY_TASK_ROOT 环境变量配置，仅保留当前项目所需的 AUTOJS6 环境变量

**章节来源**
- [AGENTS.md:99-107](file://AGENTS.md#L99-L107)
- [openspec/project.md:32-39](file://openspec/project.md#L32-L39)

### 工具价值与使用场景

**适用场景：**
- 需要频繁进行图像识别和模板匹配的AutoJS6开发者
- 需要在多设备间进行兼容性测试的脚本作者
- 希望提升开发效率，减少调试时间的自动化工程师
- 需要可视化工具辅助进行UI元素定位和交互的测试人员

**核心价值：**
- **效率提升**：从"试错式"开发转变为"可视化"开发
- **质量保证**：实时预览匹配结果，避免重复调试
- **跨设备兼容**：支持多分辨率测试，一次调试多设备适配
- **代码质量**：自动生成符合AutoJS6规范的代码，避免常见错误

### ADB设备管理功能

**设备发现与管理**
工具通过 ADB 服务实现设备的自动发现和管理：

```mermaid
graph TD
ADB["ADB服务"] --> Devices["设备列表"]
Devices --> Device1["设备1<br/>Serial: xxx<br/>Model: xxx<br/>State: device"]
Devices --> Device2["设备2<br/>Serial: yyy<br/>Model: yyy<br/>State: device"]
```

**图表来源**
- [Core/Models/AdbDevice.cs:6-37](file://Core/Models/AdbDevice.cs#L6-L37)
- [Infrastructure/Adb/AdbServiceImpl.cs:51-54](file://Infrastructure/Adb/AdbServiceImpl.cs#L51-L54)

**章节来源**
- [Core/Models/AdbDevice.cs:1-37](file://Core/Models/AdbDevice.cs#L1-L37)
- [Infrastructure/Adb/AdbServiceImpl.cs:1-54](file://Infrastructure/Adb/AdbServiceImpl.cs#L1-L54)