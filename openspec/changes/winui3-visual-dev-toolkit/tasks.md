...
- [ ] 9.1 创建 Views/CanvasView.xaml 与 CanvasView.xaml.cs
- [ ] 9.2 实现 CanvasControl 初始化（Win2D 设备创建）
- [ ] 9.3 实现分层渲染管线（CanvasImageLayer + CanvasOverlayLayer）
- [ ] 9.4 实现 CanvasBitmap 缓存池（避免重复创建纹理）
- [ ] 9.5 实现图像层渲染（位图缩放/平移/旋转）
- [ ] 9.6 实现 Overlay 层渲染（控件边界框、匹配结果框、裁剪区域框）
- [ ] 9.7 实现 60FPS 渲染优化（仅重绘变化图层）

## 10. App 层：画布交互实现

- [ ] 10.1 实现滚轮缩放（以光标为中心，范围 10%-500%）
- [ ] 10.2 实现拖拽平移（鼠标右键拖拽）
- [ ] 10.3 实现惯性滑动（释放鼠标后继续滑动并衰减）
- [ ] 10.4 实现边界阻尼回弹（超出边界时施加阻尼并回弹）
- [ ] 10.5 实现交互式矩形裁剪（拖拽创建、调整顶点/边、Shift 锁定宽高比）
...
- [ ] 11.1 实现控件边界框绘制（按类型着色：蓝色=Text、绿色=Button、橙色=Image、灰色=其他）
- [ ] 11.2 实现透明度调节（滑块控制边界框透明度）
- [ ] 11.3 实现显示过滤开关（按控件类型显示/隐藏边界框）
...
- [ ] 12.1 创建 Views/AdbDeviceList.xaml 与 AdbDeviceList.xaml.cs
- [ ] 12.2 实现设备列表显示（序列号、型号、状态、连接类型）
- [ ] 12.3 实现刷新设备按钮（触发设备扫描）
- [ ] 12.4 实现设备选择（点击高亮、设置为默认设备）
- [ ] 12.5 实现 TCP/IP 连接输入框（IP 地址 + 端口）
- [ ] 12.6 实现日志面板（实时流式输出 ADB 命令结果）

## 13. App 层：属性面板与代码预览

- [ ] 13.1 创建 Views/PropertyPanel.xaml（显示选中控件完整属性）
- [ ] 13.2 实现一键复制坐标按钮
- [ ] 13.3 实现一键复制 XPath 按钮
- [ ] 13.4 创建 Views/CodePreviewView.xaml（代码预览面板）
- [ ] 13.5 实现 JavaScript 语法高亮
- [ ] 13.6 实现手动编辑代码功能
- [ ] 13.7 实现一键复制代码按钮（Ctrl+S）
- [ ] 13.8 实现导出为 JS 文件功能
...
- [ ] 15.1 创建 Views/MainPage.xaml 主页面布局
- [ ] 15.2 实现左侧设备列表区域
- [ ] 15.3 实现中央画布区域
- [ ] 15.4 实现右侧 TreeView + 属性面板区域
- [ ] 15.5 实现底部状态栏（缩放比例、视口偏移、原始尺寸、当前模式）
- [ ] 15.6 实现顶部工具栏（截图、拉取 UI 树、生成代码、匹配测试、导出模板）
- [ ] 15.7 实现 Mica 背景与 Fluent Design 样式
