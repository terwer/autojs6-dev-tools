...
- [x] 16.2 实现匹配结果可视化（绘制绿色/黄色/橙色矩形框，标注置信度）
- [x] 16.3 实现匹配测试报告输出（耗时、坐标、状态、置信度）
...
- [ ] 22.1 重新设计 MSIX 证书与签名方案，确保普通用户下载后具备可执行的安装路径
- [ ] 22.2 验证 CI 中的 MSIX 签名、校验、证书分发与安装说明
- [ ] 22.3 仅在 x64 / ARM64 MSIX 都通过端到端安装验证后，再恢复到 `manual-release-test` 与 `release-please`
